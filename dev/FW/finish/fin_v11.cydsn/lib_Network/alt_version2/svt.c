    		...
    		case READ_ETX:
    		{
    			if (InByte == ETX)
    			{
                    ...
                    else
                    {
                        Data->EndPacket = 0;
                        ERR_CODE = 10;
                        SW_UART_PC_PutString("INVALID CHECKSUM\r\n");
                    }
...
{
	uint8_t Res = 0;;
		SW_UART_PC_PutString("Packet Recieved\r\n");
		switch (Data->Command)
		{
			case RDY:
			{
				SW_UART_PC_PutString("Packet Ok Sended\r\n");
				sprintf(buff, "%02X:%016X%04X", OKR, 0x0, 0x0);
				SW_UART_PC_PutString("Data Sended: ");
                SW_UART_PC_PutString(buff);
            ...
            case FIN_RDY_REQ:
			{
                uint32_t readystat = 0x0;
                uint16_t skiOnWay = SkierOnWay();
				SW_UART_PC_PutString("Packet Rdy Sended\r\n");
                if (GateIsOpen())
                {
                    readystat = 0x1;
                }
                else if (skiOnWay >= MAX_SKIERS_ON_WAY)
                {
                    readystat = 0x2;
                    skiOnWay = 0xFFFF;
                }
				sprintf(buff, "%02X:%016X%04X", (RDY_RESP), readystat, skiOnWay);
				SW_UART_PC_PutString("Data Sended: ");
                SW_UART_PC_PutString(buff);
                SW_UART_PC_PutString("\r\n");
				*RX = RESP_TO_SEND;
            ...
            case GTM:
            {
                ...
                SW_UART_PC_PutString("NTP SYNC Packet Recv\r\n");
                sprintf(buff, "%02X:%08X%08X%04X", (GET_TIME_RESP), UnixHigh, UnixLow, RTCgetRecentMs());
                *RX = RESP_TO_SEND;
                Res = 1;
                break;
            }
            ...
            {
                SW_UART_PC_PutString("Skier Started Packet Recv\r\n");
                sprintf(buff, "%02X:%016X%04X", (OKR), 0x0, 0x0);
                WriteStartTime(Data->Data1, Data->Data2);
                *RX = RESP_TO_SEND;
                Res = 1;
                break;
            }
			case OKR:
			{
				SW_UART_PC_PutString("Packet Ok Recieved\r\n");
				*RX = RESP_ALL_OK;
				Res = 1;
				break;
			}
