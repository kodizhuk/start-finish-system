...
#include "UART_P_PVT.h"

#if(UART_P_PIN_STATIC_MODE == 1u)
    uint32 UART_P_pinNumber = UART_P_PIN_NUMBER;
    uint32 UART_P_pinPortNumber = UART_P_PIN_PORT_NUMBER;
    #if(CY_PSOC3)
        uint32 pdata UART_P_pinDrAdress = UART_P_PIN_DR;
    #else
        uint32 UART_P_pinDrAdress = UART_P_PIN_DR;
    #endif /* (CY_PSOC3) */
#else
    uint32 UART_P_pinNumber;
    uint32 UART_P_pinPortNumber;
    #if(CY_PSOC3)
        uint32 pdata UART_P_pinDrAdress;
    #else
        uint32 UART_P_pinDrAdress;
    #endif /* (CY_PSOC3) */
    ...
    *******************************************************************************/
    void UART_P_StartEx(uint8 port, uint8 pin) 
    {
        ...
        if ((pin <= UART_P_MAX_PIN_NUMBER) && (port <= UART_P_MAX_PORT_NUMBER))
        {
            #if (!CY_PSOC4)
                portConfigAddr = UART_P_PORT_CNF_BASE;
                portConfigAddr += ((uint32)port * (UART_P_MAX_PIN_NUMBER + 1u)) + pin;
                CyPins_SetPinDriveMode(portConfigAddr, CY_PINS_DM_STRONG);
                CyPins_SetPin(portConfigAddr);
                UART_P_pinDrAdress = portConfigAddr;
            #else
                portConfigAddr = UART_P_PORT_CNF_BASE + (UART_P_PORT_CNF_SIZE * port) +
                                                                                UART_P_PORT_CNF_MODE_OFFSET;
                CY_SYS_PINS_SET_DRIVE_MODE(portConfigAddr, pin, CY_SYS_PINS_DM_STRONG);
                portDataRegAddr = UART_P_PORT_CNF_BASE + (UART_P_PORT_CNF_SIZE * port) +
                                                                                UART_P_PORT_CNF_DR_OFFSET;
                CY_SYS_PINS_SET_PIN(portDataRegAddr, pin);
                UART_P_pinDrAdress = portDataRegAddr;
            #endif /* (!CY_PSOC4) */
            UART_P_pinNumber = pin;
            UART_P_pinPortNumber = port;
        }
    }
#endif /* (UART_P_PIN_STATIC_MODE == 1u) */
...
/*******************************************************************************
* Function Name: UART_P_PutString
********************************************************************************
*
* Summary:
...
void UART_P_PutArray(const uint8 array[], uint32 byteCount) 
{
    ...
    {
        UART_P_PutChar(array[arrayIndex]);
    }
}


/*******************************************************************************
* Function Name: UART_P_PutHexByte
********************************************************************************
*
* Summary:
...
{
    static char8 const CYCODE UART_P_hex[] = "0123456789ABCDEF";

    UART_P_PutChar((uint8) UART_P_hex[txHexByte >> UART_P_BYTE_UPPER_NIBBLE_SHIFT]);
    UART_P_PutChar((uint8) UART_P_hex[txHexByte & UART_P_BYTE_LOWER_NIBBLE_MASK]);
}


/*******************************************************************************
* Function Name: UART_P_PutHexInt
********************************************************************************
*
* Summary:
...
{
    UART_P_PutHexByte((uint8)(txHexInt >> UART_P_U16_UPPER_BYTE_SHIFT));
    UART_P_PutHexByte((uint8)(txHexInt & UART_P_U16_LOWER_BYTE_MASK));
}


/*******************************************************************************
* Function Name: UART_P_PutCRLF
********************************************************************************
*
* Summary:
...
{
    UART_P_PutChar(0x0Du);
    UART_P_PutChar(0x0Au);
}
