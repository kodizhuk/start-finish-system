...
#include "SW_UART_P_PVT.h"

#if(SW_UART_P_PIN_STATIC_MODE == 1u)
    uint32 SW_UART_P_pinNumber = SW_UART_P_PIN_NUMBER;
    uint32 SW_UART_P_pinPortNumber = SW_UART_P_PIN_PORT_NUMBER;
    #if(CY_PSOC3)
        uint32 pdata SW_UART_P_pinDrAdress = SW_UART_P_PIN_DR;
    #else
        uint32 SW_UART_P_pinDrAdress = SW_UART_P_PIN_DR;
    #endif /* (CY_PSOC3) */
#else
    uint32 SW_UART_P_pinNumber;
    uint32 SW_UART_P_pinPortNumber;
    #if(CY_PSOC3)
        uint32 pdata SW_UART_P_pinDrAdress;
    #else
        uint32 SW_UART_P_pinDrAdress;
    #endif /* (CY_PSOC3) */
...
#else
    /*******************************************************************************
    * Function Name: SW_UART_P_StartEx
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void SW_UART_P_StartEx(uint8 port, uint8 pin) 
    {
        ...
        if ((pin <= SW_UART_P_MAX_PIN_NUMBER) && (port <= SW_UART_P_MAX_PORT_NUMBER))
        {
            #if (!CY_PSOC4)
                portConfigAddr = SW_UART_P_PORT_CNF_BASE;
                portConfigAddr += ((uint32)port * (SW_UART_P_MAX_PIN_NUMBER + 1u)) + pin;
                CyPins_SetPinDriveMode(portConfigAddr, CY_PINS_DM_STRONG);
                CyPins_SetPin(portConfigAddr);
                SW_UART_P_pinDrAdress = portConfigAddr;
            #else
                portConfigAddr = SW_UART_P_PORT_CNF_BASE + (SW_UART_P_PORT_CNF_SIZE * port) +
                                                                                SW_UART_P_PORT_CNF_MODE_OFFSET;
                CY_SYS_PINS_SET_DRIVE_MODE(portConfigAddr, pin, CY_SYS_PINS_DM_STRONG);
                portDataRegAddr = SW_UART_P_PORT_CNF_BASE + (SW_UART_P_PORT_CNF_SIZE * port) +
                                                                                SW_UART_P_PORT_CNF_DR_OFFSET;
                CY_SYS_PINS_SET_PIN(portDataRegAddr, pin);
                SW_UART_P_pinDrAdress = portDataRegAddr;
            #endif /* (!CY_PSOC4) */
            SW_UART_P_pinNumber = pin;
            SW_UART_P_pinPortNumber = port;
        }
    }
#endif /* (SW_UART_P_PIN_STATIC_MODE == 1u) */
...
/*******************************************************************************
* Function Name: SW_UART_P_PutString
********************************************************************************
*
* Summary:
...
void SW_UART_P_PutString(const char8 string[]) 
{
    ...
    {
        SW_UART_P_PutChar((uint8)current);
...
/*******************************************************************************
* Function Name: SW_UART_P_PutArray
********************************************************************************
*
* Summary:
...
void SW_UART_P_PutArray(const uint8 array[], uint32 byteCount) 
{
    ...
    {
        SW_UART_P_PutChar(array[arrayIndex]);
    }
}


/*******************************************************************************
* Function Name: SW_UART_P_PutHexByte
********************************************************************************
*
* Summary:
...
{
    static char8 const CYCODE SW_UART_P_hex[] = "0123456789ABCDEF";

    SW_UART_P_PutChar((uint8) SW_UART_P_hex[txHexByte >> SW_UART_P_BYTE_UPPER_NIBBLE_SHIFT]);
    SW_UART_P_PutChar((uint8) SW_UART_P_hex[txHexByte & SW_UART_P_BYTE_LOWER_NIBBLE_MASK]);
}


/*******************************************************************************
* Function Name: SW_UART_P_PutHexInt
********************************************************************************
*
* Summary:
...
{
    SW_UART_P_PutHexByte((uint8)(txHexInt >> SW_UART_P_U16_UPPER_BYTE_SHIFT));
    SW_UART_P_PutHexByte((uint8)(txHexInt & SW_UART_P_U16_LOWER_BYTE_MASK));
}


/*******************************************************************************
* Function Name: SW_UART_P_PutCRLF
********************************************************************************
*
* Summary:
...
{
    SW_UART_P_PutChar(0x0Du);
    SW_UART_P_PutChar(0x0Au);
}
