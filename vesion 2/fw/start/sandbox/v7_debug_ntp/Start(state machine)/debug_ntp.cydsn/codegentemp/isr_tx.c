...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_tx__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_tx__INTC_NUMBER];
}
...
void isr_tx_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_tx__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_tx_INTC_PRIOR = (*isr_tx_INTC_PRIOR & (uint32)(~isr_tx__INTC_PRIOR_MASK)) |
...
uint8 isr_tx_GetPriority(void)
{
    ...
    priority = (*isr_tx_INTC_PRIOR & isr_tx__INTC_PRIOR_MASK) >> priorityOffset;
