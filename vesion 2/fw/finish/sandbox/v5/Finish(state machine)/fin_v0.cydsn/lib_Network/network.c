    ...
    for(i=0; (i < NUM_TRY_SYNC) && (noConnect < NUM_TRU_SEND_PACKET); )
    {
        ...
        while((resultReceive == NO_READ) && (numAttempsReseive < NUM_CONNECT_ATTEMPS*4))
        {
            ...
            if((resultReceive == READ_OK) && (IDreceivePacket == IDpacket))
            {
                ...
                #ifdef DEBUG_INFO
                    sprintf(uartBuff, "calc time=%u:%u\n\r",sumTime, sumMillis);
                    SW_UART_DEBUG_PutString(uartBuff);
                #endif
                CyDelay(1000);
...
uint32_t SendRealTimeToStart(void)
{
    ...
    #ifdef DEBUG_INFO     
        SW_UART_DEBUG_PutString("send real time\n\r");
    #endif
...
void NTPcalculateTime(uint32_t unixTime[], uint16_t msTime[], int32_t *sumTime, int32_t *sumMs)
{
    ...
    tmpTimeMaster = unixTime[T2] - unixTime[T1];
    tmpMsMaster = (msTime[T2] - msTime[T1]);
    if(tmpMsMaster < -1000)
    {
        tmpMsMaster += 1000;
        tmpTimeMaster--;
    ...
    tmpTimeSlave = unixTime[T4] - unixTime[T3];
    tmpMsSlave = msTime[T4] - msTime[T3];
    if(tmpTimeSlave > 1)
    {
        tmpTimeSlave += 1000;
    }
