...
#include "start.h"
    ...
    #define start_SetP4PinDriveMode(shift, mode)  \
    do { \
        start_PC =   (start_PC & \
                                (uint32)(~(uint32)(start_DRIVE_MODE_IND_MASK << \
                                (start_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (start_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define start_SetP4PinDriveMode(shift, mode)  \
        do { \
            start_USBIO_CTRL_REG = (start_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(start_DRIVE_MODE_IND_MASK << \
                                    (start_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (start_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		start_SetP4PinDriveMode(start__0__SHIFT, mode);
    }
#endif
...
void start_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  start_INTCFG & (uint32)(~(uint32)position);
    start_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
