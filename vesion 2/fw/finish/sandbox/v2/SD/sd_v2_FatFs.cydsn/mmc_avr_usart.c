...
#define CS_LOW()	ChipSelect()    	/* MMC_CS = low */
#define	CS_HIGH()	ChipDeselect()		/* MMC_CS = high */
...
#define	FCLK_SLOW()	UBRR1L = 0	        /* Set slow clock (400kHz) */
...
)
{
	...
	case ISDIO_READ:
		...
		if (send_cmd(CMD48, 0x80000000 | (DWORD)sdi->func << 28 | (DWORD)sdi->addr << 9 | ((sdi->ndata - 1) & 0x1FF)) == 0) {
			for (/*Timer1 = 100*/; (rc = xchg_spi(0xFF)) == 0xFF /*&& Timer1*/; ) CyDelay(100);;
			if (rc == 0xFE) {
