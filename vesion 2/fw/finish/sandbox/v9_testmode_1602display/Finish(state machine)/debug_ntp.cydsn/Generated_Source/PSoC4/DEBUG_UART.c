...
static void DEBUG_UART_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void DEBUG_UART_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            DEBUG_UART_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            DEBUG_UART_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (DEBUG_UART_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_PIN_DM_OD_LO;
        #elif (DEBUG_UART_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX] = DEBUG_UART_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX] = DEBUG_UART_PIN_DM_OD_LO;
        #else
        #endif /* (DEBUG_UART_RX_SDA_MOSI_PIN) */
        
        #if (DEBUG_UART_TX_SCL_MISO_PIN)
            hsiomSel[DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_PIN_DM_OD_LO;
        ...
        {
        #if (DEBUG_UART_RX_SDA_MOSI_PIN)
            hsiomSel[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (DEBUG_UART_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX] = DEBUG_UART_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (DEBUG_UART_RX_SDA_MOSI_PIN) */
        
        #if (DEBUG_UART_TX_SCL_MISO_PIN)
            hsiomSel[DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (DEBUG_UART_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_PIN_DM_DIG_HIZ;
                pinsDm[DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= DEBUG_UART_TX_SCL_MISO_PIN_MASK;
            #endif /* (DEBUG_UART_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_PIN_DM_STRONG;
                pinsDm[DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_PIN_DM_DIG_HIZ;
            ...
            #if (DEBUG_UART_RX_SDA_MOSI_PIN)
                pinsInBuf |= DEBUG_UART_RX_SDA_MOSI_PIN_MASK;
            #elif (DEBUG_UART_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (DEBUG_UART_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (DEBUG_UART_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (DEBUG_UART_TX_SCL_MISO_PIN)
                hsiomSel[DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [DEBUG_UART_TX_SCL_MISO_PIN_INDEX] = DEBUG_UART_PIN_DM_OD_LO;
            #endif /* (DEBUG_UART_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (DEBUG_UART_RX_SDA_MOSI_PIN)
                    hsiomSel[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [DEBUG_UART_RX_SDA_MOSI_PIN_INDEX] = DEBUG_UART_PIN_DM_DIG_HIZ;
                #elif (DEBUG_UART_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX] = DEBUG_UART_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX] = DEBUG_UART_PIN_DM_DIG_HIZ;
                #else
                #endif /* (DEBUG_UART_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        DEBUG_UART_SET_HSIOM_SEL(DEBUG_UART_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       DEBUG_UART_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX]);

        DEBUG_UART_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[DEBUG_UART_RX_SDA_MOSI_PIN_INDEX]);

        #if (!DEBUG_UART_CY_SCBIP_V1)
            DEBUG_UART_SET_INP_DIS(DEBUG_UART_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         DEBUG_UART_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & DEBUG_UART_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!DEBUG_UART_CY_SCBIP_V1) */
        ...
        DEBUG_UART_SET_HSIOM_SEL(DEBUG_UART_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        DEBUG_UART_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        DEBUG_UART_SET_INP_DIS(DEBUG_UART_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     DEBUG_UART_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & DEBUG_UART_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        DEBUG_UART_SET_INCFG_TYPE(DEBUG_UART_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        DEBUG_UART_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (DEBUG_UART_TX_SCL_MISO_PIN)
        DEBUG_UART_SET_HSIOM_SEL(DEBUG_UART_TX_SCL_MISO_HSIOM_REG,
                                       DEBUG_UART_TX_SCL_MISO_HSIOM_MASK,
                                       DEBUG_UART_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[DEBUG_UART_TX_SCL_MISO_PIN_INDEX]);

        DEBUG_UART_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[DEBUG_UART_TX_SCL_MISO_PIN_INDEX]);

    #if (!DEBUG_UART_CY_SCBIP_V1)
        DEBUG_UART_SET_INP_DIS(DEBUG_UART_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     DEBUG_UART_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & DEBUG_UART_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!DEBUG_UART_CY_SCBIP_V1) */
    #endif /* (DEBUG_UART_RX_SDA_MOSI_PIN) */
