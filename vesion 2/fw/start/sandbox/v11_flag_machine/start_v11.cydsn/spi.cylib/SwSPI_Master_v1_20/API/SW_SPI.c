...
{
   `$INSTANCE_NAME`_CS_DR_REG |= `$INSTANCE_NAME`_CS;
   `$INSTANCE_NAME`_SCK_DR_REG &= ((uint8)(~`$INSTANCE_NAME`_CLK));
   `$INSTANCE_NAME`_MOSI_DR_REG &= ((uint8)(~`$INSTANCE_NAME`_MOSI));
}
...
void `$INSTANCE_NAME`_SendByte(uint8 byte) `=ReentrantKeil("`$INSTANCE_NAME`_SendByte")`
{
    ...
    for (countBites = BITES_ON_ONE_BYTE; countBites > 0; countBites--)
    {
        ...
        {
            `$INSTANCE_NAME`_MOSI_DR_REG |= `$INSTANCE_NAME`_MOSI;
        }
        else
        {
            `$INSTANCE_NAME`_MOSI_DR_REG &= ((uint8)(~`$INSTANCE_NAME`_MOSI));
        }  
        `$INSTANCE_NAME`_SCK_DR_REG |= `$INSTANCE_NAME`_CLK;
...
uint8 `$INSTANCE_NAME`_RecvByte(void) `=ReentrantKeil("`$INSTANCE_NAME`_RecvByte")`
{
    ...
    for (countBites = BITES_ON_ONE_BYTE; countBites > 0; countBites--)
    {
        `$INSTANCE_NAME`_MOSI_DR_REG |= `$INSTANCE_NAME`_MOSI;
        `$INSTANCE_NAME`_SCK_DR_REG |= `$INSTANCE_NAME`_CLK;
        tmpPin = (uint8)((`$INSTANCE_NAME`_MISO_PS_REG & `$INSTANCE_NAME`_MISO_MASK) >> `$INSTANCE_NAME`_MISO_SHIFT);
        if (tmpPin == 1u)
        {
